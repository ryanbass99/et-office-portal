 ...
 *
 * IMPORTANT: This version ALWAYS wipes existing data first (every run).
 * - Deletes ALL docs in openSalesOrderLines
 * - Deletes ALL docs in openSalesOrderStats
 * Then re-imports from the CSV.
 *
 * Run:
...
// ✅ ALWAYS WIPE FIRST (every run)
const ALWAYS_WIPE = true;
 ...
 * (Firestore has no "delete collection" API — you must delete docs.)
 */
async function wipeCollection(db, collectionName) {
  ...
  while (true) {
    ...
    process.stdout.write(
      `Wiping ${collectionName}... deleted ${totalDeleted}\r`
    );
  }

  process.stdout.write(`Wiping ${collectionName}... deleted ${totalDeleted}\n`);
  return totalDeleted;
}
...
(async () => {
  ...
  if (ALWAYS_WIPE) {
    console.log(
      `ALWAYS_WIPE enabled -> deleting existing docs in: ${LINES_COLLECTION} and ${STATS_COLLECTION}`
    );
    await wipeCollection(db, LINES_COLLECTION);
    await wipeCollection(db, STATS_COLLECTION);
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    // accumulate stats
    const s = stats.get(salespersonNo) || {
      orders: new Set(),
      lines: 0,
      qty: 0,
    };
    ...
    /**
     * Doc ID:
     * If your CSV has a stable line key, use it to avoid duplicates forever.
     * Otherwise fall back to rowIndex.
     */
    const lineKey =
      cleanStr(get("LineKey")) ||
      cleanStr(get("LineSeqNo")) ||
      cleanStr(get("DetailSeqNo")) ||
      cleanStr(get("SequenceNo")) ||
      cleanStr(get("LineNo")) ||
      "";

    const docId = normalizeDocId(
      lineKey
        ? `${salesOrderNo}__${lineKey}`
        : `${salesOrderNo}__${itemCode}__${i + 1}`
    );
    ...
    const payload = {
      ...
      orderDate: orderDate
        ? admin.firestore.Timestamp.fromDate(orderDate)
        : null,
  ...
  console.log("\nDone.");
  console.log("Lines collection:", LINES_COLLECTION);
  console.log("Stats collection:", STATS_COLLECTION);
})().catch((err) => {
